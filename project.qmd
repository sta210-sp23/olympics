...
```{r warning = FALSE, message = FALSE, tidy = TRUE, echo = FALSE, tidy.opts=list(width.cutoff=25)}
olympics04 %>%
  count(medal) %>%
  mutate(per = n/sum(n)) %>%
  ggplot()
...
```{rwarning = FALSE, message = FALSE, tidy = TRUE, echo = FALSE, tidy.opts=list(width.cutoff=25)}
