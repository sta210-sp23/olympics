...
```{r}
 logit_mod <- glm(goldMedal ~ sex  + age + height +  weight + noc, data = olympics_goldMedal, family = "binomial"(link = "logit")) 
   tidy(logit_mod) %>%
...
ordMod <-
  polr(factor(medals) ~ sex  + age + height +  weight + noc, data = olympics_ord , method = "probit")
tidy(ordMod) %>%
  mutate(estimate = round(estimate , 9))
```
